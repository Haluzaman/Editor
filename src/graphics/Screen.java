...
public class Screen {
    ...
    public Screen(int width, int height){
        ...
        this.renderWidth = (width >> CONFIG.TILE_WIDTH_BYTE) + 1;
        this.renderHeight = (height >> CONFIG.TILE_HEIGHT_BYTE) + 1;
...
//                    break;
    ...
    public void renderMap(Map tileMap) {
        int row = (this.y>> CONFIG.TILE_HEIGHT_BYTE);
        int column = (this.x>>CONFIG.TILE_WIDTH_BYTE);
        this.renderHeight =((this.y + this.height) >> CONFIG.TILE_HEIGHT_BYTE)+1;
        this.renderWidth = ((this.x + this.width) >> CONFIG.TILE_WIDTH_BYTE)+1;
    ...
    public void drawStraightLine(int x, int y,int targetX,int targetY,Color c,boolean isVertical){
        if(isVertical){
            if(targetY >= this.height)
                targetY = this.height - 1;
            for(int i = y; i < targetY;i++){
                this.pixels[x + i*this.width] = c.getRGB();
            }
        }else{
            if(targetX >= this.width)
                targetX = this.width - 1;
            for(int i = x; i < targetX;i++){
                if(i + y*this.width == 81920){
//                    System.out.println();
                    return;
                }
                this.pixels[i + y*this.width] = c.getRGB();
            }
        }
    }
